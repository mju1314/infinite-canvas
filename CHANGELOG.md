...
+ [修复] Gemini 生图请求使用原生 `generationConfig.imageConfig` 传递比例和尺寸，避免中转站拒绝旧的 `responseFormat` 字段。
+ [修复] Grok 视频改用 JSON 创建并适配任务响应与鉴权下载，支持单张首帧且保留刷新恢复和手动获取任务状态。
+ [修复] Grok 视频限制仅支持横屏（16:9）、竖屏（9:16）和方形（1:1）比例，避免使用不支持的宽屏或长屏比例导致 422 错误。
